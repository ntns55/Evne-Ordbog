...
\usepackage{graphicx}
\usepackage{tikz}
\usepackage{calc} % <-- important (allows 0.06\paperwidth arithmetic)
...
% Background fills page; foreground sits on a clean "card" and is fitted (no crop, no distortion)
% #1 = margin fraction (default 0.06), #2 = image file
\newcommand{\illustrationpage}[2][0.06]{%
  \clearpage
  \thispagestyle{empty}%

  % margin lengths (decimal factors are OK with calc)
  \setlength{\illumarW}{#1\paperwidth}%
  \setlength{\illumarH}{#1\paperheight}%

  % card dimensions
  \setlength{\illucardW}{\paperwidth - 2\illumarW}%
  \setlength{\illucardH}{\paperheight - 2\illumarH}%

  \begin{tikzpicture}[remember picture,overlay]
    % Background (fills page; backdrop only)
...
% Optional: rotate the whole thing for a landscape asset you want rotated
\newcommand{\illustrationpagelandscape}[2][0.06]{%
  \clearpage
  \thispagestyle{empty}%
  ...
  \begin{tikzpicture}[remember picture,overlay]
    \node at (current page.center) {%
      \rotatebox{90}{\includegraphics[width=\paperheight,height=\paperwidth]{#2}}%
    };
...
\mdtheorem[style=naturstyle]{runering}{Runering}
\mdtheorem[style=Ritualstyle]{runehjelm}{Runehjelm}
...
\mdtheorem[style=Orleksarvstyle]{nVogt}{Naturens Vogter}
...
\mdtheorem[style=Åndensgavestyle]{nkaos}{Naturens Vrede}
...
\mdtheorem[style=magiMesterstyle]{lærmagi}{Magus Magi}
